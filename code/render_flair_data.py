...
class RenderFlairData:
    ...
    def __init__(self):
        ...
        self.flair_model.final_df_copy.index = pd.to_datetime(self.flair_model.final_df_copy.index)
        #drop the column:
        self.data.drop(['Volume'],axis=1)
        self.data['Volume'] = self.flair_model.final_df_copy['scores_sum']
        self.data.dropna(inplace=True)
    ...
    def plot_points_on_graph(self):
        ...
        mpf.plot(data_df, type='candle', title="S&P 500 Data using Flair Model",ylabel_lower="Sentiment", style='charles', mav=3, volume=True, addplot=apd, figratio=(18, 10))
        #mpf.savefig('Flair_Model_Price_Sentiment.png')
