...
class Textblob_Model:
    ...
    def __init__(self):
        ...
        self.polarity = []
        self.get_polartiy()
        #turn polarity into a dataframe
        self.polarity_df = pd.DataFrame(self.polarity, columns=['compound'])
        self.final_df = pd.concat([self.dates_df,self.polarity_df],axis=1)
        ...
        self.sentiment_and_labels.dropna(inplace=True)
    ...
    def get_polartiy(self):
        sentences = self.get_headline_values()
        text = ' '.join(sentences)
        blob = TextBlob(text)
        for sentence in blob.sentences:
            self.polarity.append(round(sentence.sentiment.polarity,3))
